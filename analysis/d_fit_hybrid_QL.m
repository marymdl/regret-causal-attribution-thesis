...
%% ============================================================
%  Diagnose why "more subjects favor M3" but the paired t-test on
%  AIC(M2)-AIC(M3) is not significant - likely a few outlier subjects
%  (bad local optima in the 5-parameter M3 fit) are inflating variance.
%  Requires results_aic from fit_hybrid_QL.m (or QL_model_results.mat).
% ============================================================

diff_M2_M3 = results_aic(:,2) - results_aic(:,3);  % positive => M3 better

fprintf('======== Per-subject AIC(M2)-AIC(M3), sorted ========\n');
[sorted_diff, sort_idx] = sort(diff_M2_M3);
for i = 1:length(sorted_diff)
    fprintf('Subject %-3d : diff = %8.2f\n', sort_idx(i), sorted_diff(i));
end

fprintf('\nMean diff   = %.2f\n', mean(diff_M2_M3));
fprintf('Median diff = %.2f  <-- compare this to the mean\n', median(diff_M2_M3));
fprintf('SD diff     = %.2f\n', std(diff_M2_M3));

%% ---- flag outliers (more than 2 SD from the mean, or just very negative) ----
outlier_thresh = mean(diff_M2_M3) - 2*std(diff_M2_M3);
outliers = find(diff_M2_M3 < outlier_thresh);
fprintf('\nSubjects with unusually NEGATIVE diff (M3 fit much worse than M2):\n');
disp(outliers');
fprintf(['These are candidates for a bad local optimum in the M3 fit - try refitting ' ...
    'JUST these subjects with more restarts (e.g. n_restarts=20-30) and see if their ' ...
    'AIC(M3) improves.\n']);

%% ---- robust alternative: Wilcoxon signed-rank test (not sensitive to outliers) ----
fprintf('\n======== Wilcoxon signed-rank test (robust to outliers) ========\n');
[p_wilcoxon, ~, stats_w] = signrank(results_aic(:,2), results_aic(:,3));
fprintf('p = %.5f (signed-rank statistic = %.1f)\n', p_wilcoxon, stats_w.signedrank);
fprintf(['If this is significant while the t-test is not, it confirms the direction ' ...
    '(M3 better for most subjects) is real, and the t-test result is being distorted ' ...
    'by a handful of outliers - not evidence against the M3 hypothesis overall.\n']);

%% ---- histogram
figure('Color','w');

[counts, centers] = hist(diff_M2_M3, 15);
bar(centers, counts, 'FaceColor', [0.7 0.7 0.7]);
hold on;

yl = ylim;


line([0 0], yl, 'Color', 'k', 'LineStyle', '--', 'LineWidth', 1.5);


m = mean(diff_M2_M3);
line([m m], yl, 'Color', 'r', 'LineWidth', 2);


med = median(diff_M2_M3);
line([med med], yl, 'Color', 'b', 'LineWidth', 2);

legend({'Histogram','Zero','Mean','Median'}, 'Location', 'Best');

xlabel('AIC(M2) - AIC(M3)');
ylabel('Count');
title('Distribution of per-subject AIC difference (M2 vs M3)');
grid on;
box on;
