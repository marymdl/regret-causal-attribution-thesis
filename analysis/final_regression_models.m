...
% because Trial numbers reset at the start of each session (part 1 and
% part 2 both run 1..N)
...
%% ============================================================
%  WITHIN-SUBJECT Z-SCORING of predictor variables
% ============================================================
...
for b = 1:length(predictor_base_names)
    for L = 1:length(lags)
        predictor_cols{end+1} = [predictor_base_names{b} lags{L}]; %#ok<SAGROW>
    end
end

% single (non-lagged) continuous predictors, if present in T
...
%% ---- (2) do the z-scoring, subject by subject ----
...
% go into NEW columns with a "_z" suffix - the original raw columns are
...
for c = 1:length(predictor_cols)
    ...
    for i = 1:length(subj_list)
        ...
        if sd == 0 || isnan(sd)
            ...
            % e.g. a lag that's constant for them) - z-score is
            % undefined, so just set to 0 rather than dividing by zero
            T.(z_col)(m) = 0;
        else
...
%% ---- example: refit your core model with z-scored predictors ----
...
%% ============================================================
%  Does the global (weak-looking) correlation hide a much stronger
%  LOCAL/CONDITIONAL collinearity within the subset where Regret>0?
% ============================================================
...
fprintf(['If the "Regret>0 only" correlation is close to 1 while the "ALL trials" ' ...
    'correlation is much weaker, this confirms the rectification (max(0,.)) is diluting ' ...
    'a near-perfect LOCAL dependency into a moderate-looking GLOBAL Pearson correlation - ' ...
    'exactly why the linear correlation/VIF diagnostics looked mild while the regression ' ...
    'coefficients were unstable.\n']);
%% ============================================================
